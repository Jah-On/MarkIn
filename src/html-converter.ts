  ...
  implements ICstNodeVisitor<null, Element>
{
  ...
  markin(children: MarkinCstChildren, _param?: null | undefined): Element {
    ...
    let functionArg = this.visit(children.functionArg);
    return functionArg;
  }
  functionCall(
    ...
    const parseFunctionArgString = (
      arg: FunctionArgCstNode
    ...
    const parseFunctionArg = (
      argType: FunctionArgumentType,
      arg: FunctionArgCstNode
    ): Result<any, string[]> => {
      if (argType.t === "string") {
        return parseFunctionArgString(arg);
      } else if (argType.t === "number") {
        return parseFunctionArgString(arg).then((value) => {
        ...
        return Ok.allOk(
          arrayContents.map((arrayContent) =>
            parseFunctionArg(argType.v, arrayContent)
          )
        ).mapError((errors) => {
          return errors.flat();
        });
    ...
    const myFunctionArgs = Ok.allOk(
      myFunction.args.map((argType, i) =>
        parseFunctionArg(argType, functionArg[i])
      )
    );
